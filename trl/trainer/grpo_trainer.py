...
class GRPOTrainer(Trainer):
    ...
    def _generate_and_score_completions(self, inputs: list[dict[str, Any]]) -> list[dict[str, Any]]:
        ...
        for node_idx, node in enumerate(tree):
            ...
            if node.get("split", 0) > 1 and not node.get("dont_calc_loss"):
                ...
                child_indices = [i for i, n in enumerate(tree) if n["parent_idx"] == node_idx]
    ...
    def compute_loss(self, model, inputs_list, return_outputs=False, num_items_in_batch=None):
        ...
        for i, inputs in enumerate(inputs_list):
            ...
            per_token_logps = self._get_per_token_logps(model, input_ids.to(self.model.device), attention_mask.to(self.model.device), logits_to_keep.to(self.model.device))
