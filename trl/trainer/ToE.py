...
MIN_THOUGHT_TOKENS = 15

SPLITABLE_TOKENS = {"\n", "!", ".", "?"}  # thought separators
MAX_SPLIT = 6  # same as MAX_SPLIT in discussion
...
class TreeNode:
    ...
    def to_dict(self) -> Dict[str, Any]:
        """Recursively export the subtree rooted at this node to a JSON-serialisable dict."""
        return {
            "depth": self.depth,
            ...
            "thought_quota": self.thought_quota,
...
class TreeOfThoughtsEntropyVLLM:
    ...
    def _reward(self, txt: str, ans: str) -> float:
        try:
            return int(math_equal(extract_final_answer(txt), ans))
        except:
            return 0
    ...
    async def _seed_first_two_children(self, root: TreeNode, answer: str):
        ...
        async for chunk in self.engine.generate(self.tokenizer.decode(root.prompt_ids), params,
                                                request_id=str(uuid.uuid4())):
            out = chunk.outputs[0]
            tok_id = out.token_ids[-1]
            top = {tid: -99 if np.isinf(e.logprob) else e.logprob for tid, e in out.logprobs[-1].items()}
    ...
    async def _spawn(self, node: TreeNode, answer: str):
        async with self.sem:
            ...
            node.prompt_text = prefix_text
            thoughts_indices = []
            async for chunk in self.engine.generate(prefix_text, params, request_id=str(uuid.uuid4())):
                out = chunk.outputs[0]
                if node.first_pass:
                    if chunk.finished or (hasattr(chunk,'finish_reason') and chunk.finish_reason is not None):
                        thoughts_count = len(thoughts_indices) + 1
                        start_index = int(np.ceil(max(0, (thoughts_count - MAX_SPLIT) / 2)))
                        node.prompt_ids = node.prompt_ids + list(out.token_ids[:thoughts_indices[start_index]])
                        node.first_pass = False

                        await self._resume_stream(node, answer)
                        return

                    if out.text[-1] in SPLITABLE_TOKENS:
                        total_out_tokens = len(out.token_ids)
                        if thoughts_indices and (total_out_tokens - thoughts_indices[-1]) <= MIN_THOUGHT_TOKENS:
                            continue
                        thoughts_indices.append(total_out_tokens)
                    continue


                top = {tid: e.logprob for tid, e in out.logprobs[-1].items() if
                       not np.isinf(e.logprob)} if out.logprobs else {}
                H = _token_entropy(list(top.values()))
                if (
                        H > TAU and out.text[-1] in SPLITABLE_TOKENS and
                        node.depth < (MAX_SPLIT + 1) and len(out.token_ids) > MIN_THOUGHT_TOKENS
                ):
                    node.completion_ids = list(out.token_ids)
                    self._entropy_branch(node, out, top, answer)
                    return
    ...
    async def _entropy_branch(self, node: TreeNode, out, top, answer):
        ...
        probs = np.exp(cand_lps);
        probs /= probs.sum();
        alt_id = int(np.random.choice(cand_ids, p=probs))
        for forced in (tok_id, alt_id):
